...
describe("queryflags query", function()
    ...
    it("does not group a bare quoted phrase -- quotes only matter after a flag colon", function()
...
describe("queryflags escaped colon as a flag escape", function()
    it("treats a boolean flag with an escaped colon as query text", function()
        local r = qf.parse(schema, "is\\:fixed hello")
        assert.is_nil(r.flags.fixed)
        assert.are.equal("is:fixed hello", r.query)
    end)

    it("treats a key:value token with an escaped colon as query text", function()
    ...
    it("does not open a value's quoted span for a plain quote outside flag position", function()
        -- '"' is only special right after a flag's colon; elsewhere it's a
        -- literal character with no grouping and no closing requirement.
        local r = qf.parse(schema, '"is:fixed" "path:foo"')
        assert.is_nil(r.flags.fixed)
        assert.is_nil(r.flags.path)
        assert.is_nil(r.error)
        assert.are.equal('"is:fixed" "path:foo"', r.query)
    end)
end)
...
describe("queryflags value flags", function()
    ...
    it("resolves \\\" to a literal quote in query text even outside flag position", function()
        -- the outer quotes are plain characters here (not preceded by a flag
        -- colon), so they're kept as-is; only the escaped inner quotes resolve.
    ...
    it("treats a bare leading quote as a literal character, not an unterminated span", function()
        -- no flag colon precedes it, so '"' never opens a span here -- there's
        -- nothing to leave unclosed.
...
describe("queryflags highlight", function()
    ...
    it("gives a bare quote no special highlight -- it's just a character now", function()
        -- '"is:fixed"' is query text (the colon never opens a value span, since
        -- neither quote follows it directly), so the quote chars get no
        -- Keyword/String/Delimiter highlight at all -- they're plain text.
        local hls = qf.highlight(schema, '"is:fixed"')
        for _, h in ipairs(hls) do
            assert.is_true(h.hl ~= "Keyword" and h.hl ~= "String" and h.hl ~= "Delimiter")
        end
    end)
    ...
    it("dims the backslash of an escaped colon", function()
        ...
        local nontext = {}
        for _, h in ipairs(hls) do
            if h.hl == "NonText" then table.insert(nontext, h) end
        end
        -- the '\' sits at byte 2, right before the escaped ':'
        assert.are.same({
            { start = 2, finish = 3, hl = "NonText" },
        }, nontext)
    end)
end)
