...
`:bdelete` closes every window showing the buffer, taking your splits with it.
This module points those windows at another buffer first, then deletes.
...
Buffers kept this way are reported as a count, separately from buffers kept for
unsaved changes; only the second is a warning.
...
out of its windows (a `BufUnload` autocmd, `'confirm'`), it is put back rather
than left orphaned.
...
`:bdelete` unlists and unloads the buffer but keeps its number and marks, so
reopening the file lands you back where you were. `:bwipeout` frees the number
and drops the marks. Use `:BWipeout` for a stale terminal, a renamed file or a
session reset, and `:BDelete` otherwise.
