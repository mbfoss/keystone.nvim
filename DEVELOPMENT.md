...
[`tests/`](tests/), where they are discovered through [`.busted`](.busted).
They run through [`tests/nvim-lua`](tests/nvim-lua), so each spec executes
inside a real Neovim and can use the `vim` API.
...
modules build on: floating/fixed/input windows, a tree buffer, an LRU cache,
throttle/debounce, timers, a signal type, string/fs utilities, process
spawning, spinners, and user-command registration. Prefer
...
`keystone.unsaved.session`) instead of loading it; loading fires the whole
