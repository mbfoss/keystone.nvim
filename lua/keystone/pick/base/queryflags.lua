...
local function _tokenize(str)
    ...
    while i <= len do
        ...
        local prev_colon      = false -- true right after an unescaped ':' -- the only spot
                                       -- where a '"' is allowed to open a value's quoted span
        ...
        while i <= len do
            ...
            elseif c == "\\" and str:sub(i + 1, i + 1) == '"' then
                -- outside a value's quoted span \" is just a literal double quote.
                table.insert(_escapes, i - tok_start + 1)
                table.insert(chars, '"')
                i          = i + 2
                prev_colon = false
            elseif c == "\\" and str:sub(i + 1, i + 1) == ":" then
                -- \: is a literal colon that never acts as the key/value
                -- delimiter, so the token it's part of stays plain query text.
                table.insert(_escapes, i - tok_start + 1)
                table.insert(chars, ":")
                i          = i + 2
                prev_colon = false
            elseif c:match("%s") then
                break
            elseif c == '"' and prev_colon then
                -- a '"' only opens a value's quoted span right after the
                -- colon that introduces it; anywhere else it is just text.
...
-- matching a value def; value may be quoted); a boolean flag is "is:flagname" matching a
-- boolean def. Else query text -- escaping the colon (key\:value) forces query text even
-- if the key would otherwise look like a flag.
