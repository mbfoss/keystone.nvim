--- Persistent, letter-keyed bookmarks for the file explorer (files or
--- directories). Independent from the file tree's marks store: it keeps its own
--- char -> path map and persists to a separate file.
local M = {}
...
local function _filepath()
    return _persist_file or vim.fs.joinpath(vim.fn.stdpath("data"), "keystone.exploremarks.json")
end

--- Rebuild the path -> char reverse index from _marks. When several chars point
--- at the same path the last one seen wins, which is all the display needs.
